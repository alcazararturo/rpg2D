...
public class Inventario : Singleton<Inventario>
{
    ...
    [SerializeField] private int numeroDeSlot;
    [SerializeField] private Personaje personaje;

    public int NumeroDeSlots => numeroDeSlot;    
    public InventarioItem[] ItemsInventario => itemsInventario;
    public Personaje Personaje => personaje;

    private void Start()
    {
        itemsInventario = new InventarioItem[numeroDeSlot];
    }
                ...
                for (int i=0; i < indexes.Count; i++)
                {
                    if (itemsInventario[indexes[i]].Cantidad < itemPorAnadir.AcumulacionMax)
                    {
                        ...
                        InventarioUI.Instance.DibujarItemEnInventario(itemPorAnadir, 
                                                                     itemsInventario[indexes[i]].Cantidad,
                                                                     indexes[i]);
                        return;
                    }
                }
    ...
    {
        if (itemsInventario[indexInicial] == null || itemsInventario[indexFinal] != null) { return; }
    ...
    {
        if (itemsInventario[index] == null) { return; }
        if (itemsInventario[index].tipo != TiposDeItem.Armas) { return; }
